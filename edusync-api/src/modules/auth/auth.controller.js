...
export const register = async (req, res) => {
  ...
  try {
    if (!nombre || !apellidos || !correo || !contrasena || !tipo_usuario) {
    ...
    const [result] = await pool.query(
      `INSERT INTO Usuario (Nombre, Apellidos, Correo, Contrasena, Tipo_Usuario, Programa) 
       VALUES (?, ?, ?, ?, ?, ?)`,
      [nombre, apellidos, correo, hashedPassword, tipo_usuario, programa || null]
    );
